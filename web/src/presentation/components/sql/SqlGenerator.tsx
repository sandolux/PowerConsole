...
import { Clipboard, Play, ScanBarcode } from 'lucide-react'; // A+¦adir ScanBarcode
...
import { LabelViewer } from '../tools/LabelViewer';

export const SqlGenerator = ({ workspaceId }: { workspaceId: string }) => {
  const {
    selectedProfileId,
    ...
    logExecutionUseCase,
    deleteLog,
    reloadLogs, // Add reloadLogs here
  ...
  useEffect(() => {
    ...
    if (!isAllowed) {
      ...
      setProfileWarning(
        'El perfil previamente seleccionado no est+í permitido para este Template. Selecciona uno de la lista.'
      );
    } else {
      setProfileWarning(null);
    }
  ...
  const handleOpenSqlRunner = useCallback(() => {
    openSqlRunnerModal(generatedScript, selectedProfileId);
  }, [openSqlRunnerModal, generatedScript, selectedProfileId]);
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-2">
          ...
          {templates.length === 0 && (
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                No hay templates definidos. Ve a la pesta+¦a{' '}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
        {/* +ürea 1: Input */}
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-2">
          <div className="flex items-center justify-between mb-2"> {/* mb-2 a+¦adido para espacio */}
            ...
            <button
              ...
              title="Importar c+¦digos de barra desde Label Viewer"
            >
        ...
        {/* +ürea 2: Configuraci+¦n de Contexto */}
        <div className="space-y-3 border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-800">
          ...
          {contextParams.length === 0 && (
            <p className="text-sm text-gray-600 dark:text-gray-400">No hay par+ímetros de contexto (todos son batch o no existen).</p>
          )}
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 space-y-2">
          ...
          <textarea
            ...
            placeholder="-- El SQL generado aparecer+í aqu+¡..."
          />
        </div>
      </div>
      ...
      <Modal
        ...
        title="Importar C+¦digos de Barra"
