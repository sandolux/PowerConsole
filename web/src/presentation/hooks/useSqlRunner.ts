...
export const useSqlRunner = (workspaceId: string) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const script = await generateUseCase.execute(selectedTemplateId, inputData, workspaceId, contextValues, useLoopMode);
      setGeneratedScript(script);
    } catch (error) {
      setGeneratedScript(`-- Error generating script: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }, [contextValues, inputData, selectedTemplateId, templateRepo, scriptGenerator, workspaceId, useLoopMode]);
